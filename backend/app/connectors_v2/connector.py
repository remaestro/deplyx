...
from app.connectors_v2.probes import ProbeCredentials, select_collection_probe
...
class UnifiedConnector:
    ...
    async def sync(self) -> dict[str, Any]:
        ...
        try:
            ...
            # A probe may own collection for this device (e.g. Cisco FTD via
            # the FDM/OAuth API). The unified profile engine handles the rest.
            collection_probe = select_collection_probe(fp, self.connector_type)
            if collection_probe is not None:
                data = await collection_probe.collect(self._probe_creds())
                result.update(data)
    ...
    def _probe_creds(self) -> ProbeCredentials:
    ...
    def _discover(self) -> dict[str, Any]:
        # If a probe owns collection for this device, let it identify (OAuth)
        # rather than attempting an SSH fingerprint that would fail.
        probe = select_collection_probe({"os": ""}, self.connector_type)
        if probe is not None and self.transport_mode != "ssh" and self.api_username and self.api_password:
            identity = probe.identify(self._probe_creds())
